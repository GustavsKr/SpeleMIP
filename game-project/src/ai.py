...
def _moves(c1: int, c2: int, c3: int, c4: int) -> List[int]:
    ...
    m.sort(key=lambda x: _PRIORITY[x], reverse=True)
    return m


def _apply_pick(c1: int, c2: int, c3: int, c4: int,
                p1: int, p2: int, player: int, pick: int) -> State:
    if pick == 1:
        c1 -= 1
        if player == 1: p2 += 1
        else: p1 += 1
...
def alphabeta(state: State, depth: int, alpha: int, beta: int,
              memo: Dict[Tuple[State, int], int]) -> int:
    key = (state, depth)
    if key in memo:
        return memo[key]
    ...
    if player == 1:
        # Player 1 wants maximize value
        value = -10**9
        for m in possible:
            nxt = _apply_pick(c1, c2, c3, c4, p1, p2, player, m)
            value = max(value, alphabeta(nxt, depth - 1, alpha, beta, memo))
            alpha = max(alpha, value)
            if alpha >= beta:
                break
    else:
        # Player 2 wants minimize value
        value = 10**9
        for m in possible:
            nxt = _apply_pick(c1, c2, c3, c4, p1, p2, player, m)
...
def choose_move(sequence: List[int], p1: int, p2: int, current_player: int, depth: int = 8) -> int:
    ...
    root: State = (c1, c2, c3, c4, p1, p2, current_player)
    ...
    if current_player == 1:
        ...
        for m in possible:
            nxt = _apply_pick(c1, c2, c3, c4, p1, p2, current_player, m)
            val = alphabeta(nxt, depth - 1, alpha, beta, memo)
            if (val > best_val) or (val == best_val and _PRIORITY[m] > _PRIORITY[best_move]):
                best_val = val
                best_move = m
